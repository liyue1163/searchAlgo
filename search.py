...
def depthFirstSearch(problem):
    ...
    while not OPEN.isEmpty():
        ...
        for successor in successors:
            checkList = []
            for step in path:
                checkList.append(step[0])
            newState = successor[0]
            if not newState in checkList:
                nextPath = path[:]
                nextPath.append(successor)
                OPEN.push(nextPath)
    print("Open is empty, no solution")
    # util.raiseNotDefined()
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not OPEN.isEmpty():
        ...
        for successor in successors:
            ...
            for i in range(1, len(nextPath)):
                actions.append(nextPath[i][1])
            cost = problem.getCostOfActions(actions) + heuristic(successor[0], problem)
            if not newState in seen or cost < seen[newState]:
                seen[newState] = cost
                OPEN.push(nextPath, cost)
    print("Open is empty, no solution")
