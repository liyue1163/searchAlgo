    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                nextPos = (nextx, nexty)
                cost = 1
                cornerStateCopy = list(state[1])
                if nextPos in self.corners:
                    index = self.corners.index(nextPos)
                    cornerStateCopy[index] = 1
                nextState = (nextPos, tuple(cornerStateCopy))
                successors.append( ( nextState, action, cost) )
...
def cornersHeuristic(state, problem):
    ...
    while len(unvisited) != 0:
        ...
        for corner in unvisited:
            # Compute manhattan distance between current position and
            # next nearest unvisited corner
...
def foodHeuristic(state, problem):
    ...
    for food in foodList:
        if foodGrid[food[0]][food[1]] == True:
            foodRest.append(food)

    if len(foodRest) == 0:
        return heuristicCost

    if len(foodRest) == 1:
        heuristicCost = mazeDistance(position, foodRest[0], problem.startingGameState)
        return heuristicCost

    else:
        distStore = util.PriorityQueue()
        for food in foodRest:
            dist = mazeDistance(position, food, problem.startingGameState)
            distStore.push(food, -dist)

        farestFood = distStore.pop()
        farestFood2 = distStore.pop()
    diff = mazeDistance(farestFood, farestFood2, problem.startingGameState)
    heuristicCost = diff + mazeDistance(position, farestFood2, problem.startingGameState)
    return heuristicCost
